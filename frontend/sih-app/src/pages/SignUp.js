...
import { registerUser } from "../api/users"; // implement registerUser POST API
...
export default function SignUp() {
  ...
  return (
    <div style={{ maxWidth: 400, margin: "auto", padding: spacing.l, background: colors.cardBg, borderRadius: 12, marginTop: spacing.l }}>
      <h2 style={{ fontSize: typography.title, color: colors.primary, marginBottom: spacing.m }}>Sign Up</h2>
      <input type="text" placeholder="Name" value={name} onChange={(e)=>setName(e.target.value)} style={{ width: "100%", padding: spacing.m, marginBottom: spacing.m, borderRadius: 8, border: `1px solid ${colors.border}` }} />
      ...
      <button onClick={handleSubmit} style={{ width: "100%", padding: spacing.m, background: colors.secondary, color: "#fff", border: "none", borderRadius: 8, fontWeight: 600 }}>Sign Up</button>
    </div>
  );
}
